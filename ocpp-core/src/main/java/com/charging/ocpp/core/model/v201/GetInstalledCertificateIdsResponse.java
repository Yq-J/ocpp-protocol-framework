 ...
 * <p>
 * 用途：承载 GetInstalledCertificateIds 操作的响应字段，用于查询已安装证书标识场景下的 OCPP CALL/CALLRESULT payload 序列化与反序列化。
...
public class GetInstalledCertificateIdsResponse {
     ...
     * 厂商自定义扩展数据。
     * <p>
     * 用途：对应 OCPP 字段 {@code customData}，在 OCPP 2.0.1 GetInstalledCertificateIdsResponse 协议对象中传递厂商自定义扩展数据。
     ...
     * 状态补充信息。
     * <p>
     * 用途：对应 OCPP 字段 {@code statusInfo}，在 OCPP 2.0.1 GetInstalledCertificateIdsResponse 协议对象中传递状态补充信息。
     ...
     * 证书哈希链数据。
     * <p>
     * 用途：对应 OCPP 字段 {@code certificateHashDataChain}，在 OCPP 2.0.1 GetInstalledCertificateIdsResponse 协议对象中传递证书哈希链数据。
     * 字段类型为 {@code List<CertificateHashDataChain>}，用于承载一组证书哈希链数据。该字段在官方规范中为可选字段，未提供时由业务语义或对端默认行为决定。数组至少包含 1 个元素。
     * </p>
     */
