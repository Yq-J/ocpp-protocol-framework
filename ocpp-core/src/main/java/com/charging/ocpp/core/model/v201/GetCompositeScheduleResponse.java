...
/**
 * OCPP 2.0.1 的 GetCompositeSchedule 响应 payload 协议实体类。
 * <p>
...
public class GetCompositeScheduleResponse {
     ...
     * 厂商自定义扩展数据。
     * <p>
     * 用途：对应 OCPP 字段 {@code customData}，在 OCPP 2.0.1 GetCompositeScheduleResponse 协议对象中传递厂商自定义扩展数据。
     ...
     * 处理状态。
     * <p>
     * 用途：对应 OCPP 字段 {@code status}，在 OCPP 2.0.1 GetCompositeScheduleResponse 协议对象中传递处理状态。
     ...
     * 状态补充信息。
     * <p>
     * 用途：对应 OCPP 字段 {@code statusInfo}，在 OCPP 2.0.1 GetCompositeScheduleResponse 协议对象中传递状态补充信息。
     ...
     * 计划。
     * <p>
     * 用途：对应 OCPP 字段 {@code schedule}，在 OCPP 2.0.1 GetCompositeScheduleResponse 协议对象中传递计划。
     * 字段类型为 {@code CompositeSchedule}，用于承载计划。该字段在官方规范中为可选字段，未提供时由业务语义或对端默认行为决定。具体合法性由官方 JSON Schema 和业务状态机共同约束。
     * </p>
     */
