 ...
 * <p>
 * 用途：描述Set监控数据相关的结构化数据，作为 OCPP 2.0.1 请求或响应 payload 中的嵌套对象复用。
...
public class SetMonitoringData {
     ...
     * 厂商自定义扩展数据。
     * <p>
     * 用途：对应 OCPP 字段 {@code customData}，在 OCPP 2.0.1 SetMonitoringData 协议对象中传递厂商自定义扩展数据。
     ...
     * 对象标识。
     * <p>
     * 用途：对应 OCPP 字段 {@code id}，在 OCPP 2.0.1 SetMonitoringData 协议对象中传递对象标识。
     ...
     * 交易。
     * <p>
     * 用途：对应 OCPP 字段 {@code transaction}，在 OCPP 2.0.1 SetMonitoringData 协议对象中传递交易。
     * 字段类型为 {@code Boolean}，用于承载交易。该字段在官方规范中为可选字段，未提供时由业务语义或对端默认行为决定。具体合法性由官方 JSON Schema 和业务状态机共同约束。
     * </p>
     */
     ...
     * 字段值。
     * <p>
     * 用途：对应 OCPP 字段 {@code value}，在 OCPP 2.0.1 SetMonitoringData 协议对象中传递字段值。
     ...
     * severity。
     * <p>
     * 用途：对应 OCPP 字段 {@code severity}，在 OCPP 2.0.1 SetMonitoringData 协议对象中传递severity。
     ...
     * 设备组件。
     * <p>
     * 用途：对应 OCPP 字段 {@code component}，在 OCPP 2.0.1 SetMonitoringData 协议对象中传递设备组件。
     ...
     * 设备变量。
     * <p>
     * 用途：对应 OCPP 字段 {@code variable}，在 OCPP 2.0.1 SetMonitoringData 协议对象中传递设备变量。
