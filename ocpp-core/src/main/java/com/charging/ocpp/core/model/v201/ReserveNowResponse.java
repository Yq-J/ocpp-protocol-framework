...
/**
 * OCPP 2.0.1 的 ReserveNow 响应 payload 协议实体类。
 * <p>
 * 用途：承载 ReserveNow 操作的响应字段，用于创建充电预约场景下的 OCPP CALL/CALLRESULT payload 序列化与反序列化。
...
public class ReserveNowResponse {
     ...
     * 厂商自定义扩展数据。
     * <p>
     * 用途：对应 OCPP 字段 {@code customData}，在 OCPP 2.0.1 ReserveNowResponse 协议对象中传递厂商自定义扩展数据。
     ...
     * 处理状态。
     * <p>
     * 用途：对应 OCPP 字段 {@code status}，在 OCPP 2.0.1 ReserveNowResponse 协议对象中传递处理状态。
     ...
     * 状态补充信息。
     * <p>
     * 用途：对应 OCPP 字段 {@code statusInfo}，在 OCPP 2.0.1 ReserveNowResponse 协议对象中传递状态补充信息。
